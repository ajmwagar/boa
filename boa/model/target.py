...
import re
from ..tools.util import RepresentsInt, bcolors
...
class Target(object):
    ...
    def to_str(self, show_bssid=False):
        ...
        if 'WEP' in encryption:
            encryption = str(bcolors.OKGREEN + encryption + bcolors.ENDC)
        elif 'WPA' in encryption:
            encryption = str(bcolors.FAIL + encryption + bcolors.ENDC)
        ...
        if self.power > 50:
            ...
            color = bcolors.FAIL
        power = str(color + power + bcolors.ENDC)

        wps = ('{O} n/a')
        if self.wps == True:
            wps = ( bcolors.OKGREEN + 'yes' + bcolors.ENDC)
        elif self.wps == False:
            wps = (bcolors.FAIL +'no' + bcolors.ENDC)
        else:
            wps = ('n/a')

        clients = '       '
        if len(self.clients) > 0:
            clients = str(bcolors.OKGREEN + str(len(self.clients)) + bcolors.ENDC)

        result = '%s  %s %s  %s  %s  %s' % (
                essid, bssid, encryption, power, wps, clients)
        return result
